...
$("#btnAddgiveawayCompetition").click(function () {
...
function validateCompition() {
    var validator = $("#giveawaycompition").validate({
        rules: {
            Email: {
                required: true,
                email: true
...
function addCompitionUser() {
    ...
    model.Name = $("#name").val();
    model.Promoname = "giveawayNov2020";
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == true) {
                $("#giveawaycompition").trigger("reset");
                $('#mainsection').hide();
                $('#thankssection').show();
                $('#btncompetitonspinner').hide();
            }
...
function isRegistered() {
    ...
    $.ajax({
        ...
        url: "/isregistered",
        dataType: 'json',
        data: { Email: $("#email").val(), Promoname: "giveawayNov2020"  },
        success: function (data) {
            if (data.status == true) {
                addCompitionUser();
            } else {
