using Microsoft.Extensions.Options;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Net.Http.Headers;
using System.Text;
using System.Text.Json;
using System.Threading.Tasks;
using THPromotionPortal.Models.ApiContracts.Response;
using THPromotionPortal.Models.AuthUsers;
using THPromotionPortal.Models.Configuration;
using THPromotionPortal.Models.enums;
using THPromotionPortal.Models.ViewModel;
using THPromotionPortal.Services.General;
using THPromotionPortal.Services.Interfaces;

namespace THPromotionPortal.Services.Implementations
{
    public class PromotionService : IPromotionService
    {
        private readonly BasicAuthSettings _basicAuthSettings;
        private readonly IHttpClientFactory _clientFactory;
        private readonly IAuthAccess _authAccess;
        private readonly EndPoints _endPoints;

        public PromotionService(IHttpClientFactory clientFactory, IAuthAccess authAccess, IOptions<EndPoints> endPoints, IOptions<BasicAuthSettings> basicAuthSettings)
        {
            _basicAuthSettings = basicAuthSettings.Value;
            _clientFactory = clientFactory;
            _authAccess = authAccess;
            _endPoints = endPoints.Value;
        }

        public async Task<dynamic> CreatePromotion(CreatePromotionViewModel model)
        {

            var Apiresponse = await Post("/Promotion/CreatePromotion", model, AuthType.BasicAuth);
            return Apiresponse;
        }
        public async Task<dynamic> GetAllPromotion()
        {
            var Apiresponse = await Get("/Promotion/GetPromotions", AuthType.BasicAuth);
            if (Apiresponse.IsSuccessStatusCode)
            {
                var responseJson = JsonConvert.DeserializeObject<GenericApiResponse<List<PromotionResponseDTO>>>(JObject.Parse(Apiresponse.Content.ReadAsStringAsync().Result).ToString());
                return responseJson;
            }
            return GenericApiResponse<PromotionDatesDTO>.Failure("No Promotion Found", ApiStatusCodes.PromotionNotFound);
        }

        public async Task<HttpResponseMessage> Get(string url, AuthType authType)
        {
            var client = _clientFactory.CreateClient();
            client.BaseAddress = new Uri(_endPoints.PromotionEndPoint);


            if (AuthType.BasicAuth == authType)
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic",
                    Bas64Convertor.Base64Encode(_basicAuthSettings.Username + ":" + _basicAuthSettings.Password));
            }
            else if (AuthType.TokenBased == authType)
            {
                var authUser = _authAccess.GetProfile();

                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authUser.Token);
            }

            HttpResponseMessage response = await client.GetAsync(client.BaseAddress + url);

            return response;
        }
        public async Task<HttpResponseMessage> Post(string url, object model, AuthType authType)
        {
            string jsonString = JsonConvert.SerializeObject(model);
            var payload = new StringContent(jsonString, Encoding.UTF8, "application/json");

            var client = _clientFactory.CreateClient();
            client.BaseAddress = new Uri(_endPoints.PromotionEndPoint);


            if (AuthType.BasicAuth == authType)
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic",
                    Bas64Convertor.Base64Encode(_basicAuthSettings.Username + ":" + _basicAuthSettings.Password));
            }
            else if (AuthType.TokenBased == authType)
            {
                var authUser = _authAccess.GetProfile();

                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authUser.Token);
            }

            HttpResponseMessage response = await client.PostAsync(client.BaseAddress + url, payload);

            return response;
        }

        public async Task<dynamic> GetPromotion(int id)
        {
            var Apiresponse = await Get("/Promotion/GetPromotions" + id, AuthType.BasicAuth);
            if (Apiresponse.IsSuccessStatusCode)
            {
                var responseJson = JsonConvert.DeserializeObject<GenericApiResponse<List<PromotionResponseDTO>>>(JObject.Parse(Apiresponse.Content.ReadAsStringAsync().Result).ToString());
                return responseJson;
            }
            return GenericApiResponse<PromotionDatesDTO>.Failure("No Promotion Found", ApiStatusCodes.PromotionNotFound);
        }
    }
}
